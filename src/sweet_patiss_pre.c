...
int main(int argc, char **argv) {
    ...
    parse_ids(argv[2]);
    sscanf(argv[3], "%d %d", &shm_sweet_patiss_paste_id, &sem_sweet_patiss_paste_id);
    ...
    printf("From sweet_patiss :Current Process ID: %d\n", pid);
    return 0;
}
...
void attach_shm_basic_items() {
    ...
    if (shm_paste_ptr == (char *)-1) {
        perror("shmat paste in sweet paste failed");
        exit(1);
    }
}
...
void do_work() {
    ...
    int paste_int=modify_shared_int(sem_sweet_patiss_paste_id, shm_sweet_patiss_paste_ptr, amount*2);
    }